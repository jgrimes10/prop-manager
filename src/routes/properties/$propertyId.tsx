                ...
                <div className='rounded-md border border-gray-200 bg-white p-4 md:col-span-2'>
                    ...
                    <dl className='mt-3 grid grid-cols-1 gap-3 text-sm sm:grid-cols-2'>
                        ...
                        <div>
                            ...
                            <dd className='font-medium'>{new Date(property.updatedAt).toLocaleDateString()}</dd>
                        </div>
                    </dl>
                </div>
...
function AddUnitModal({ isOpen, onClose, propertyId }: AddUnitModalProps) {
    // On the server, document is not defined; but isOpen will be false during SSR,
    // so this will early-return and never call createPortal.
    if (!isOpen) return null;

    return createPortal(
        <div className='fixed inset-0 z-40 flex items-center justify-center bg-black/40' aria-modal='true' role='dialog'>
            <div className='relative w-full max-w-md rounded-lg bg-white p-4 shadow-lg'>
                <header className='mb-2 flex items-center justify-between gap-4'>
                    <h2 className='text-sm font-semibold text-gray-800'>Add Unit</h2>
                    <button
                        type='button'
                        onClick={onClose}
