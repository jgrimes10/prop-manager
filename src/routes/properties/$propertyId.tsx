...
import type { CreateUnitInput, Unit } from "@/domain/unit";
...
const UNIT_STATUS_MAP: Record<Unit['status'], { label: string; variant: StatusVariant }> = {
    vacant: { label: "Vacant", variant: "warning" },
    occupied: { label: "Occupied", variant: "success" },
    notice: { label: "Notice Given", variant: "error" },
                ...
                {units.length === 0 ? (
                    ...
                    <div className="mt-3 overflow-x-auto">
                        <table className="min-w-full text-left text-sm">
                            ...
                            <tbody>
                                {units.map((unit) => {
                                    const config = UNIT_STATUS_MAP[unit.status];
